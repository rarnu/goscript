...
func TestRun(t *testing.T) {
	...
	loop := NewEventLoop()
	prg, err := goscript.Compile("main.js", SCRIPT, false)
	if err != nil {
		t.Fatal(err)
	}
...
func TestStart(t *testing.T) {
	...
	loop.RunOnLoop(func(vm *goscript.Runtime) {
		vm.RunProgram(prg)
	})
...
func TestRunNoSchedule(t *testing.T) {
	loop := NewEventLoop()
	fired := false
	loop.Run(func(vm *goscript.Runtime) { // should not hang
		fired = true
...
func TestNativeTimeout(t *testing.T) {
	...
	fired := false
	loop := NewEventLoop()
	loop.SetTimeout(func(*goscript.Runtime) {
		fired = true
	}, 1*time.Second)
	loop.Run(func(*goscript.Runtime) {
		// do not schedule anything
	})
	if !fired {
		t.Fatal("Not fired")
...
func TestNativeClearTimeout(t *testing.T) {
	...
	fired := false
	loop := NewEventLoop()
	timer := loop.SetTimeout(func(*goscript.Runtime) {
		fired = true
	}, 2*time.Second)
	loop.SetTimeout(func(*goscript.Runtime) {
...
func TestNativeInterval(t *testing.T) {
	...
	loop := NewEventLoop()
	var i *Interval
	i = loop.SetInterval(func(*goscript.Runtime) {
		t.Log("tick")
		count++
		if count > 2 {
...
func TestRunOnStoppedLoop(t *testing.T) {
	...
	go func() {
		for atomic.LoadInt32(&failed) == 0 {
			loop.RunOnLoop(func(*goscript.Runtime) {
				if !loop.running {
...
func TestEventLoop_StopNoWait(t *testing.T) {
	...
	loop := NewEventLoop()
	var ran int32
	loop.Run(func(runtime *goscript.Runtime) {
		loop.SetTimeout(func(*goscript.Runtime) {
