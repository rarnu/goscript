    ...
    `, "(anonymous): Line 3:9 Illegal break statement")
		{
			...
			is(p.position(stmt.From).Column, 9)
			is(p.position(stmt.To).Column, 16)
			is(p.slice(stmt.From, stmt.To), "break; ")
		}
...
func Test_parseNumberLiteral(t *testing.T) {
	tt(t, func() {
		...
		test("0", 0)
		test("0x8000000000000000", 9.223372036854776e+18)
	})
}

func TestPosition(t *testing.T) {
	tt(t, func() {
		p := newParser("", "// Lorem ipsum")
		is(p.slice(0, 1), "")
		is(p.slice(0, 10), "")
		is(p.slice(1, 128), "")
		is(p.str[0:0], "")
		...
		is(p.str[0:1], "/")
		is(p.slice(1, 2), "/")
		is(p.str[0:14], "// Lorem ipsum")
		is(p.slice(1, 15), "// Lorem ipsum")
		p = newParser("", "(function(){ return 0; })")
		program, err := p.parse()
		is(err, nil)
		var node ast.Node
		...
		is(p.slice(node.Idx0(), node.Idx1()), "function(){ return 0; }")
		is(p.slice(node.Idx0(), node.Idx1()+1), "function(){ return 0; })")
		is(p.slice(node.Idx0(), node.Idx1()+2), "")
		is(node.(*ast.FunctionLiteral).Source, "function(){ return 0; }")
		node = program
		is(node.Idx0(), file.Idx(2))
		is(node.Idx1(), file.Idx(25))
		is(p.slice(node.Idx0(), node.Idx1()), "function(){ return 0; }")
		p = newParser("", "(function(){ return abc; })")
		program, err = p.parse()
		is(err, nil)
		node = program.Body[0].(*ast.ExpressionStatement).Expression.(*ast.FunctionLiteral)
